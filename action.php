...
if(isset($_POST['step'])) {
    ...
    if($_SESSION['needs'] < 22) {
        if($_SESSION['needs'] >= 10){
            if(($_SESSION['needs'] == 10) && (rand(1, 100) >= 10)){
                ...
                <img src="img/war_peasant.jpg"><br>';
            }
    ...
    if(isset($_SESSION['distance'])) {
        ...
        if($_SESSION['distance'] <= 0) {
            if($_SESSION['warriors'] < $_SESSION['enemy']){
                ...
                <br><img src="img/lose_war.jpeg"><br>';
            } else {
    ...
    if(rand(1, 3) == 1) {
        ...
        $notice .= '<p>Крысы сожрали '.$rats.' бушелей зерна.</p><br>
            <img src="img/rats.jpg"><br>';
    }

    //Лазутчик
    if($_SESSION['warriors'] < (($_SESSION['people'] + $_SESSION['warriors'] + $_SESSION['farmer']) / 10)){
        if(rand(1, 4) == 1){
            $spy = rand($_SESSION['grain']/10, $_SESSION['grain']/4);
            $_SESSION['grain'] = $_SESSION['grain'] - $spy;
            $notice .= '<p>Вражеский лазутчик испортил '.$spy.' бушелей зерна.</p><br>
            <img src="img/spy.jpg"><br>';
        }
    }

    //Пожар
    if(rand(1, 8) == 1){
        $fire = rand($_SESSION['grain']/4, $_SESSION['grain']/2);
        $_SESSION['grain'] = $_SESSION['grain'] - $fire;
        $notice .= '<p>Пожар. Сгорело '.$fire.' бушелей зерна.</p><br>
        <img src="img/fire.jpg"><br>';
    }
...
if(isset($_POST['calc'])) {
    ...
    if(isset($_POST['f_val'], $_POST['f_action'])) {
        if($_POST['f_action'] == '+') {
            if(($_POST['f_val'] > ($_SESSION['people'] + $_SESSION['warriors'] + $_SESSION['farmer'])) || (($_SESSION['land'] / $_SESSION['farmer_harvest']) < ($_SESSION['farmer'] + $_POST['f_val']))) {
                $tmp_f = ($_SESSION['land'] / $_SESSION['farmer_harvest']) - $_SESSION['farmer'];
                if($tmp_f > ($_SESSION['people'] + $_SESSION['warriors'] + $_SESSION['farmer'])){
                    $tmp_f = $_SESSION['people'] + $_SESSION['warriors'] + $_SESSION['farmer'];
                }
    ...
    if(isset($_POST['bs_val'], $_POST['bs_action'])) {
        if($_POST['bs_action'] == '+') {
            ...
            } elseif($_POST['bs_val'] > ($_SESSION['land'] - ($_SESSION['farmer'] * $_SESSION['farmer_harvest']))) {
                $tmp_l = $_SESSION['land'] - ($_SESSION['farmer'] * $_SESSION['farmer_harvest']);
                $tmp_ln = '<br>Максимум можно продать:';
            } else {
                $_SESSION['grain'] = $_SESSION['grain'] + $_POST['bs_val'] * $_SESSION['land_price'];
                $_SESSION['land'] = $_SESSION['land'] - $_POST['bs_val'];
            }
...
<div>
    ...
    <div style="float: left;">
        ...
        <table border="solid 1px">
            <tr>
                <td>Год правления: <?php echo $_SESSION['year']; ?> год</td>
            </tr>
            <tr>
                ...
                <td><b>Население: <?php echo $_SESSION['people']; ?> чел.</b></td>
                <td>Земли: <?php echo $_SESSION['land']; ?> акр.</td>
            ...
            <tr>
                ...
                <td>
                    Беженцы: <?php echo $_SESSION['refugees']; ?> чел.<br>
                    Взросление: <?php echo $_SESSION['born']; ?> чел.<br>
                    Умерло: <?php echo $_SESSION['die']; ?> чел.
                </td>
                <td>Цена на землю <?php echo $_SESSION['land_price']; ?> буш/акр</td>
                <td>22 бушеля норма</td>
